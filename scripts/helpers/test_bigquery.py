"""
Test script for BigQuery connection and basic operations
"""
...
def test_bigquery_connection():
    ...
    try:
        ...
        db = BigQueryDatabase()
        logger.info("Successfully initialized BigQuery connection")
        ...
        df = db.export_to_csv({'strategy': 'test_strategy'}, 'test_trades_bigquery.csv')
        logger.info(f"Successfully exported {len(df)} trades to CSV")
        
        logger.info("All BigQuery tests completed successfully!")
...
def test_data_integrity():
    ...
    try:
        ...
        if len(retrieved_trades) >= len(test_trades):  # BigQuery might have some delay
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n✗ Tests failed: {str(e)}")
        print("\nMake sure you have:")
        print("1. BigQuery API enabled in GCP")
        print("2. Service account credentials set up")
        print("3. Installed google-cloud-bigquery package")
        print("4. Proper permissions for BigQuery") 